...
z = Superfluids.argand()
r = abs.(z)

s, d = Superfluids.default(:superfluid), Superfluids.default(:discretisation)
...
Threads.@threads for j = eachindex(rr)
    ...
    s1[j] = Superfluids.solve(s, d, qs[j], (0.0, h/2Ω); μ)
    s2[j] = Superfluids.solve(s, d, s1[j], (h/2Ω, h/Ω); μ)
    r₀ = find_vortex(qs[j])
    ss[j] = abs(r₀)
    nv[j] = sum(@. abs2(qs[j])*(r<abs(ss[j])))
    r₁ = find_vortex(s1[j])
    r₂ = find_vortex(s2[j])
