...
# Householder reflect out the zero mode

uv0 = [ψ[:]; conj.(ψ[:])]
uv1 = zero(uv0);  uv1[1] = norm(uv0)^2
v = uv1 - uv0
v /= norm(v)
# F = Matrix(I, size(BdGmat)) - 2*v*v'/norm(v)^2
# F = F[2:end,:]
# TODO check why F*uv0 has elements of order 0.01
# NB a reflection is its own inverse
# B = F*BdGmat*F'
B = copy(BdGmat)
B .-= 2*v*(v'*B)
B .-= 2*(B*v)*v'
...
ωs,uvs,nconv,niter,nmult,resid = eigs(B; nev=16, which=:SM)
uvs = [zero(uvs[1:1,:]); uvs]
uvs .-= 2*v*(v'*uvs)		# undo reflection
